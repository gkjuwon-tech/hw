...
>
> **글자 읽기 싫으면 그림만 봐도 됨.** 형이 핵심 단계마다 그림 그려놨음 (전체 흐름 / 깔판 단면 / 격자 치수 / 멀티미터 검사 / PCB 연결). 그림에 화살표랑 숫자 다 있음. 진짜 그림만 보고 따라해도 1대 나오게 해놨음. 제발 그림이라도 봐.
>
> ---
>
...
> 💡 형이 PCB 설계, SW 전부, 깔판 설계, 펌웨어, 펌웨어 ↔ 백엔드 게이트웨이, 캘리브레이션 알고리즘, 가격 모델, 웹훅, 마이그레이션, Docker, 테스트 60개+, 그리고 방금 **스트라이프 결제 연동**까지 다 끝냈음. 너 진짜로 할 거: **(a) 발주 5번 (b) 가위질 + 라미네이팅 1번 (c) PCB 도착하면 케이블 2개 꽂기 (d) Arduino IDE 에서 펌웨어 굽기 (e) `docker compose up` 한 줄.** 이 5개임. 다섯 개. 손가락으로 세어봐. 다섯 개 맞지?

> 💡 **"2~3주" 의 진실**: 그중 네가 실제로 손 움직이는 시간은 다 합쳐서 **반나절 + α** 임. 나머지는 전부 택배 기다리는 시간. 즉 너는 거의 다 "기다리는" 일을 함. 기다리는 거 잘하잖아. 형 코드 리뷰 기다리는 거 한 달도 하던데.
...
![전체 흐름도: 깔판 → Scanner PCB → 노트북 → Cloud → pass/fail. 파란 박스가 네 일, 회색이 형 일.](docs/img/01-overview.svg)
...
**형 일** = `SOFTWARE` 박스 안의 모든 것 + PCB 설계 + 깔판 설계 + 결제 + 이 문서.
**둘 다 망하면 안 되는 거** = USB-C 케이블 빠지면 둘 다 망함. 그러니까 데모 중엔 케이블에 테이프 한 조각 붙여서 고정해. 별거 아닌 거 같지? 데모 망하는 거 90%가 케이블임.

> 💡 이 그림에서 점선 위쪽 (HARDWARE) 이 네 영역, 아래쪽 (SOFTWARE) 이 형 영역. 근데 네 영역도 사실상 "발주 + 가위질" 이 전부고, 진짜 어려운 PCB 설계는 형이 했어. 그러니까 정확히 말하면 네 영역은 "HARDWARE 박스에서 형이 안 한 나머지" = 가위질 + 발주 + 케이블 꽂기. 이게 다임.

> 💡 Edge 박스 (Jetson Orin Nano) 는 첫 프로토에서 **사지 마라.** 그건 양산 직전 "고객 공장에 두고 갈 박스" 인데, 첫 데모는 너 노트북에 USB-C 한 가닥 꽂아서 충분히 됨. Jetson 은 두 번째 라인 깔 때부터. 지금 사면 30만원 더 쓰고 책상 위에서 먼지만 먹음. 사지 마. 사지 말라고 두 번 썼다.
...
> 형은 발주 자동조종 모드인데 너는 처음일 수 있음. 이미 아는 데는 건너뛰어라. 모르면 한 줄씩 따라와. 형이 진짜 손잡고 알려줌.

### 3.1 디바이스마트 (devicemart.co.kr) — 한국, 다음날

1. [devicemart.co.kr](https://www.devicemart.co.kr/) → 우측 상단 회원가입. 회사 명의로 살 거면 **사업자 회원** (세금계산서 발급용). 잘 모르면 일단 개인으로 사고 영수증 챙겨.
2. 상단 검색창 (돋보기) → 부품명 그대로 입력. 예: `Kapton tape 12mm`, `FFC 1.0mm 16pin 150mm`.
3. 원하는 거 장바구니 담기 → 우측 상단 **장바구니** → **주문하기** → 배송지 입력 → **신용카드** 결제.
4. **오후 3시 이전 결제 → 당일 출고 → 다음 영업일 도착.** 3시 넘으면 하루 밀림. 오전에 질러.
5. 세금계산서 발급 (사업자 회원이면 자동, 마이페이지에서 받음).

### 3.2 쿠팡 — 한국, 다음날 (로켓배송)

1. 그냥 사. 모르면 부모님한테 물어봐. (이건 진짜 농담 아님. 쿠팡은 진짜 누구나 함.)
2. **로켓배송** 마크 (파란 로켓) 있는 거만 골라. 일반 판매자 거 사면 1주일 걸림. 우리 일정에 1주일은 사치.
3. 회사 카드로 긁어. 영수증 (주문내역) 캡처해둬. 지출증빙 나중에 모아서 정산.

### 3.3 Adafruit (adafruit.com) — 미국, DHL 5~10일

1. [adafruit.com](https://www.adafruit.com/) → 우측 상단 회원가입 (Sign in → Create account).
2. 검색창 → 부품 번호 (예: `1361`). 번호로 검색하면 정확히 그 부품 나옴.
3. 장바구니 (Cart) → **Checkout** → 배송지 (영문, 예시 그대로 형식 맞춰):
   ```
   Name: Gildong Hong
   ...
   ```
4. 배송: **DHL Express** ($55) ← 추천. 4~5일 도착. USPS 같은 싼 거 고르면 2~3주 걸리고 통관에서 더 잘 걸림. DHL 써.
5. 결제 (VISA / Master).
6. **개인통관고유부호** 필수 입력 ([unipass.customs.go.kr](https://unipass.customs.go.kr) 에서 발급한 "P..." 13자리). 결제 단계 또는 배송 정보에 입력란 있음.
7. **합산 $150 이하면 면세.** 위 표 2.1 합산이 ~$32 이라서 면세권. 다른 거 같이 사면 합산되니 주의. "이것도 사야지~" 하고 이것저것 담다가 $150 넘기면 부가세+관세 맞음. 표에 있는 것만 사.

### 3.4 JLCPCB (jlcpcb.com) — 중국, DHL 8~10일 ⭐

> 위의 `2.2` 섹션에 클릭 순서 다 있음. 한 번 더 짧게 정리:

1. 회원가입 → **Order Now**.
2. **Gerber 업로드** → `hardware/pcb/conet-scanner-v1/gerbers.zip` (형이 별도 PR 로 합류시킨 뒤 가능. 그 전엔 이 단계 못 함.)
3. 옵션: **4-layer / ENIG / 5장 / 검정 또는 보라**.
4. **PCB Assembly** 토글 ON → **Top Side / Standard**.
5. **BOM 업로드** (`bom.csv`) + **CPL 업로드** (`cpl.csv`).
6. 빨간 줄 (재고 없음) 나오면 → Alternative 클릭 → 형이 표시한 대체부품 선택.
7. **DHL Express** 선택 → 결제 → 끝.
8. 진행 상황은 JLCPCB 사이트 "My Orders" 에서 추적. "Production" → "Quality Check" → "Shipped" 순서로 넘어감. 매일 새로고침하지 마. 하루 한 번이면 충분. 형이 너 새로고침 100번 할 거 안다.

> 💡 **첫 사용자 쿠폰**: 회원가입 직후 신규 사용자 쿠폰 ($30 정도 + 무료배송 쿠폰) 자동 적용됨. 받아먹어. 결제 직전 쿠폰란에 "available coupons" 뜨면 제일 큰 거 적용. 공돈임.

### 3.5 발주 체크리스트

- [ ] 위 표 (2.1, 2.3) 모든 항목 → 사이트별 장바구니 담음. (PCB 2.2 는 Gerber PR 머지 후)
- [ ] 디바이스마트: 세금계산서 발급 가능 회원가입.
- [ ] Adafruit / JLCPCB: 개인통관고유부호 입력란 채움.
- [ ] 영문 배송 주소 정확 (오타 = 통관 지연).
- [ ] 회사 카드 (지출증빙 캡처).
- [ ] 도착 예정일 캘린더에 박음. (안 박으면 까먹고 "왜 안 와요" 톡 함.)
- [ ] **결제 후 형한테 톡: "다 발주 완료. M1~M7 / 도구 X / Y / Z. 도착 예정 YYYY-MM-DD. PCB 는 Gerber PR 머지되면 바로 쏠게."**
  - "발주했음" 만 보내면 형이 못 알아봐. **항목 + 도착예정일** 같이 보내야 함. 이게 패턴임. 항상 "뭘 + 언제" 같이.
...
![PCB 연결 그림: 깔판의 가로줄(주황) → J1, 세로줄(파랑) → J2, PCB 의 USB-C → 노트북. J1=행, J2=열 바꿔 꽂지 말라는 경고.](docs/img/05-pcb-wiring.svg)

> 👆 **주황은 J1, 파랑은 J2, 그리고 USB-C 한 가닥 노트북에.** 색만 맞춰 꽂으면 됨. 주황(가로) → J1, 파랑(세로) → J2.
...
2. **PCB ↔ 노트북**: USB-C 케이블 한 가닥. 노트북에 꽂고, 반대쪽 보드에 꽂고. 끝.
...
### 6.4 (보너스) 결제 흐름도 살아있음 — 형이 방금 스트라이프 꽂았음
...
> 이제 깔판(센서) → 노트북(게이트웨이) → 백엔드(AI) 를 한 줄로 잇는 거. 이거 되면 데모 가능 상태임.
...
> 💡 이 스크립트도 형이 줬음. 너는 `PORT` 한 줄만 네 포트로 바꾸면 됨. 딱 한 줄. 6번째 줄. `PORT = ` 로 시작하는 줄. 그 줄만.

### 7.2 실행 (3분)

백엔드 (6번) 켜져 있는 상태에서, 새 터미널:
...
1. 라인 자동 생성됨 ("Line ... 생성됨" 출력).
2. "정상품 5개 흘려라" 라고 시킴 → 깔판 위에 **정상품** (빵 1개 또는 무거운 책 등) 을 5번 올렸다 내렸다 해. 2초마다 한 번씩 (스크립트가 알아서 텀 줌). 같은 물건이라도 위치 살짝씩 바꿔서.
3. 캘리브레이션 자동 (= "정상은 이렇게 생겼다" 를 AI 가 학습).
4. inspect 무한루프 시작. 이제 깔판 위에 뭐 올리면:
   - 정상품 = `verdict=pass`
   - 다른 거 (찌그러진 거, 빈 거, 이물질) = `verdict=fail`

이 시점 = **데모 가능.** 형한테 영상 보내. **자랑할 때임. ㅋㅋ** 정상품 올렸을 때 pass 뜨고, 이상한 거 올렸을 때 fail 뜨는 화면 + 깔판 같이 찍어서. 이게 우리 회사 첫 데모 영상이 되는 거임. 잘 찍어.

> 💡 **여기까지 왔으면 너 하드웨어 엔지니어 됨.** 한 달 전엔 LED 극성도 몰랐는데 지금 압력 센서 만들고 AI 에 연결했음. 형이 인정함. (한 번만.)
...
`docker compose logs api` 로 로그 봐 (새 터미널에서, `hw/backend` 위치에서). 99% 는:
- 라인이 calibrated 상태가 아님 → `/v1/lines/{id}/calibrate` 먼저 호출해야 함 (게이트웨이가 자동으로 함, 5개 안 흘렸으면 안 됨).
- frame 크기 안 맞음 → line 만들 때 rows/cols 가 16×16 인지 확인.
- frame 이 list 가 아니라 string → JSON 직렬화 문제. 게이트웨이 스크립트 안 건드렸으면 안 생김.
...
- [ ] 이 문서 0~3절 읽음 (30분) ← 지금 읽고 있는 거. 절반 했네.
- [ ] 개인통관고유부호 발급 (10분)
- [ ] Adafruit (M1~M3) 결제 (15분)
- [ ] 디바이스마트 (M4, M7) 결제 (10분)
- [ ] 쿠팡 (M5, M6, 도구 T1~T7) 결제 (20분)
- [ ] **JLCPCB SMT 풀턴키 5장 결제** (40분) ⭐ ← Gerber PR 머지된 후
- [ ] 회사 카드 영수증 챙김 (캡처)
- [ ] 형한테 톡: "M1~M7 / 도구 N개 발주 완료. 도착 예정 YYYY-MM-DD. PCB 는 Gerber 머지되면 바로."
...
이 시점 = **둘 다 투자자 미팅 잡을 수 있음.** 그리고 너도 이제 "하드웨어 우리가 직접 만들어요" 라고 당당히 말할 수 있음. 네 손으로 만들었으니까.
...
>
> 이 비율 봐. 형이 미친 게 아니라 너 진짜 운 좋은 거. 받은 패가 로얄 스트레이트 플러시인 거야. 너는 그냥 카드 안 떨어뜨리고 테이블까지 들고 가기만 하면 돼. 그게 이 문서가 시키는 전부임.
>
> 진짜 1대 만들고 영상 보내. 그 뒤로는 너랑 형 둘 다 투자자 앞에서 **같은 무게로** 발표함. 지분이 반반이면 일도 (적어도 이 빌드만큼은) 네가 끝까지 하는 거 보여줘. 그게 공평한 거임.
>
> 막히면 톡. **"안 됨"** 만 보내지 마. **사진 + 어디까지 + 어디서.** 세 개. 항상 세 개. 안 그러면 형도 답 못 하고, 너만 멈춰 있고, 회사만 늦어짐.
>
> 인두에 손 대지 마라. 진짜로. (세 번째 경고임. 데이는 사람 꼭 있음. 너 아니길.)
>
> 이거 다 하면 형이 진짜로 밥 산다. 한우. 약속.
>
> 가자.
>
> — 형
