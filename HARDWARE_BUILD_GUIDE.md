...
>
> **글자 읽기 싫으면 그림만 봐도 됨.** 형이 핵심 단계마다 그림 그려놨음 (전체 흐름 / 발주 지도 / 발주 타임라인 / 깔판 단면 / 격자 치수 / 멀티미터 검사 / PCB 연결 / Edge 셋업 5단계 / Edge 박스 조립 / 3D 프린트 함체 / 키오스크). 그림에 화살표랑 숫자 다 있음. 진짜 그림만 보고 따라해도 1대 나오게 해놨음. 제발 그림이라도 봐.
>
> ---
>
...
| E5 | (옵션) **방열 케이스 / 쿨링팬** | 1 | 쿠팡/총판 | ₩25,000 | Dev Kit은 방열판 기본 있음. **케이스는 2.6 의 3D 프린트 함체로 대체** — E5 살 필요 없음 |
...
### 2.6 Tactile Edge 함체 — **3D 프린트 케이스, 약 3만원** ★NEW

> "Edge **박스**" 라면서 정작 NVIDIA 개발자키트 알몸을 컨베이어 옆에 던져놓을 거임? ㅋㅋ 방열판 노출된 기판이 먼지·기름·지나가던 지게차 앞에서 떨고 있는 그림임. Cognex In-Sight 는 다이캐스팅 바디로 폼 잡는데 우린 기판 알몸이면 폼이 안 남. 그래서 **3D 프린트 함체**로 감싸고 전면에 디스플레이 박아서 "제품"으로 만든다. 설계 형이 줬음 — `hardware/edge_enclosure/edge_enclosure.scad` (파라메트릭). 프린터 없으면 STL 두 개 뽑아서 출력소 맡겨.

| # | 부품 | 수량 | 어디서 | 단가 (대략) | 검색어 / 메모 |
|---|------|------|--------|-----------:|---------------|
| C1 | **PETG 필라멘트** | ~150g | 쿠팡 | (1kg 스풀 ₩25,000 중 일부) | PLA 말고 **PETG**. Jetson 열 + 공장 햇빛에 PLA는 물러져서 처짐 |
| C2 | **M3 황동 열박음 인서트** | 8 | 쿠팡/알리 | ₩6,000 | 베젤 4 + 보드 스탠드오프 4. 인두로 데워서 박음 |
| C3 | **M3 x 8mm 나사** | 4 | 쿠팡 | ₩3,000 | 베젤 → 본체 |
| C4 | **M2.5 x 5mm 나사** | 4 | 쿠팡 | ₩3,000 | Jetson 보드 → 스탠드오프 (네 보드 나사에 맞춰) |
...
3. **케이스**: 아래 7.0.5 에서 **3D 프린트 함체**(2.6)에 넣는다. E5 방열케이스 살 필요 없이 우리 함체가 방열·보호 둘 다 함. (함체 아직 안 뽑았으면 일단 방열판 그대로 두고 7.0.5는 나중에.)
...
> 💡 **여기까지가 "부품 한 덩어리".** Jetson(+SSD) 뒤, 터치 화면 앞. 함체 안 씌우면 이 상태로도 데모는 됨. 근데 알몸이라 폼이 안 남 → 7.0.5에서 **3D 프린트 함체** 씌워서 "제품"으로 마감한다.

#### 7.0.5 3D 프린트 함체에 넣기 (제품답게, 30분) ★NEW

> 7.0.4까지가 "부품 한 덩어리"였다면, 7.0.5는 그걸 **함체에 넣어 진짜 "제품"으로** 만드는 단계. 알몸 기판 → Cognex 박스처럼. 함체 설계는 형이 줬음 (`hardware/edge_enclosure/`, 2.6에서 프린트). 프린트는 몇 시간~며칠 걸리니 **발주 주에 미리 돌려놔** (PCB·해외부품 기다리는 동안 같이).

![3D 프린트 함체 분해도: 전면 베젤(디스플레이 창) + 7" 디스플레이 + 본체(Jetson 스탠드오프 + 통풍구) + 백플레이트(VESA-75) 를 M3 나사 4개로 조여 완성 박스로.](docs/img/12-edge-enclosure.svg)

1. **인서트 박기**: 인두로 본체의 베젤 보스 4개 + 보드 스탠드오프 4개에 M3/M2.5 황동 인서트(C2)를 열박음. (인두로 데워서 살짝 눌러 넣으면 플라스틱에 박힘.)
2. **Jetson → 본체**: 보드(+SSD)를 본체 스탠드오프에 올리고 M2.5 나사(C4) 4개로 고정. 스탠드오프가 보드를 띄워서 밑으로 바람 통함 (통풍구랑 세트).
3. **디스플레이 → 베젤**: 7" 디스플레이(D1)를 전면 베젤 뒤쪽 포켓에 끼움. 창으로 화면만 보임.
4. **케이블 통과 + 닫기**: HDMI·터치USB·전원을 본체 뒤 **케이블 출구**로 빼고(실제 연결은 7.6.1), 베젤을 본체에 덮어 M3 나사(C3) 4개로 조여.
5. **거치**: 백플레이트 **VESA-75** 에 마운트암/스탠드(D6) 물려 컨베이어 옆에 세움.

> 💡 **이제 진짜 "박스".** 방열판 노출 기판 → 손바닥만한 깔끔한 콘솔: 전면 터치, 후면 VESA, 옆구리 통풍구. 투자자 앞에 들고 가도 안 부끄러움.

> 🖨 **프린터 없어?** 출력소·학교·메이커스페이스에 `edge_enclosure.scad` 에서 뽑은 STL 두 개 보내면 됨. **plan B**: 첫 데모는 7.0.4처럼 D6 스탠드 + 케이블타이로 임시 거치하고, 함체는 나중에 씌워도 데모엔 지장 없음.
...
### 8.14 "3D 프린트 함체(7.0.5)가 안 맞음"

- **보드 나사구멍이 스탠드오프랑 어긋남** → `.scad` 의 `jet_holes` 가 네 보드랑 다른 거. 자로 실측해서 4개 [x,y] 고치고 다시 출력. (이게 제일 흔함 — 그래서 베젤 먼저 test-fit 하라는 거.)
- **디스플레이가 베젤 창에 안 맞음 / 헐겁다** → `disp_w/h`, `win_w/h` 실측값으로 수정. 헐거우면 포켓에 폼테이프 한 줄.
- **인서트가 헐렁/안 들어감** → 보스 구멍 `insert_d` 조정(±0.2mm). 인두 온도 너무 높으면 주변 녹아 헐거워짐 — 200~230℃에서 천천히.
- **베젤 나사가 본체 보스랑 안 맞음** → 둘 다 같은 `boss_pos` 쓰니 보통 맞음. 휘었으면 4벽 이상으로 다시 출력(휨 방지).
- **PLA로 뽑았더니 여름 창가에서 처짐** → PETG로 다시. (그래서 C1이 PETG임.)
...
- [ ] **함체용 PETG + M3/M2.5 인서트·나사(C1~C4)** 장바구니 + **함체 프린트 시작** (며칠 걸리니 지금 ⭐NEW) — 프린터 없으면 STL 출력소 접수
...
- [ ] **3D 프린트 함체에 넣기** (7.0.5): 인서트 박고 → Jetson·디스플레이 넣고 → M3로 닫기 → VESA 거치 (30분) ⭐NEW
