...
![발주 지도: Adafruit(미국·M1~M3·면세권), 디바이스마트(한국·M4,M7), 쿠팡(한국·M5,M6,도구), JLCPCB(중국·PCB×5·제일 먼저). 합계 약 30~32만원, 전부 네 카드.](docs/img/07-procurement-map.svg)

> 👆 **네 군데서 끝.** 어느 사이트에서 뭘 사는지 이 한 장에 다 있음. 색깔로 묶어놨으니 카드별 장바구니라고 생각하고 채워.

### 3.0 돈은 누가 내냐 — **너. 네 카드. 이건 협상 대상 아님.** 💸

- 형이 한 거: 회사 기획, PCB 설계·layout, 펌웨어, 백엔드, AI, 결제 연동, 깔판 설계, 그리고 **이 발주 가이드**. 시간으로 환산하면… 계산 안 함. 무한대 나옴.
- 그러니까 **부품값 ~30만원은 네가 회사 카드로 긁어.** 형 인건비 앞에서 이건 반올림하면 0원임. "왜 내가?" 라고 묻고 싶으면 위 작업 분담 표(10절) 다시 봐. 보고 나면 안 물어볼 거임.
- 회사 카드 없으면 일단 네 개인 카드로 긁고 **주문내역 캡처 → 지출증빙으로 회사에 청구.** 결제하면서 영수증/주문번호 다 캡처해둬. 안 캡처하면 정산할 때 또 너만 손해.
- ⚠ 절대 형 카드 빌려달라고 하지 마. 그건 선 넘는 거임. (사랑하지만 선은 선임.)

### 3.0.1 발주 순서 — **오래 걸리는 것부터. 같은 날 다 질러.**

![발주 타임라인: Day 0에 네 군데 동시 발주. 한국부품 1~2일, Adafruit 4~5일, JLCPCB 8~10일(제일 김). 기다리는 동안 깔판 만들고, PCB 도착하면 케이블 꽂고 데모.](docs/img/08-order-timeline.svg)

> 👆 **PCB(주황)가 제일 오래 걸림 = 전체 일정을 PCB가 정함.** 그러니까:
> 1. **Gerber PR 머지되면 그날 즉시 JLCPCB부터** 질러 (8~10일짜리라 제일 먼저).
> 2. 같은 날 Adafruit(4~5일) + 한국 부품(1~2일)도 다 질러. 한 번에.
> 3. 한국 부품 도착하는 1~2일 안에 **깔판 만들기 시작** (4절). 어차피 PCB 기다려야 하니 그 시간에 깔판 뽑는 거임.
> 4. PCB 도착 → 케이블 꽂고 → 펌웨어 → inspect → 데모. 순서만 지키면 노는 시간 0.
>
> 거꾸로 "한국 부품 와서 깔판 다 만든 다음 PCB 시켜야지~" 하면 PCB 8~10일을 멍하니 더 기다리는 거임. 그 1~2주가 우리 회사 런웨이에서 그냥 증발함. **같은 날 다 질러.**
...
- [ ] **3.0 읽음** — 돈은 내가(co-founder) 낸다는 거 받아들임. 💸
- [ ] 개인통관고유부호 발급 (해외 발주 전에!)
- [ ] **디바이스마트**: M4 Kapton, M7 FFC ×2 결제 (오후 3시 전)
...
- [ ] **결제 후 형한테 톡** (형식 지켜):
  > "발주 완료. 디마(M4,M7) 내일 / 쿠팡(M5,M6,도구) 내일 / Adafruit(M1~M3) ~Day5 / JLCPCB(PCB×5) ~Day10. 전부 결제했고 증빙 캡처함."
